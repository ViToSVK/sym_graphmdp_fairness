...
bdd_ptr	bdd_existential_quantify (DdManager* dd, bdd_ptr f, bdd_ptr cube) {
  DdNode *result;

  /* ref is to be taken care of */
  result = Cudd_bddExistAbstract(dd, (DdNode*) f, (DdNode*) cube);
  common_error(result, "bdd_existential_quantify: result = NULL");
  Cudd_Ref(result);
  return((bdd_ptr)result);
}

bdd_ptr	bdd_universal_quantify (DdManager* dd, bdd_ptr f, bdd_ptr cube) {
  DdNode *result;

  /* ref is to be taken care of */
  result = Cudd_bddUnivAbstract(dd, (DdNode*) f, (DdNode*) cube);
  common_error(result, "bdd_universal_quantify: result = NULL");
  Cudd_Ref(result);
  return((bdd_ptr)result);
}

bdd_ptr	bdd_then (bdd_ptr f) {
  DdNode *result;

  /* ref is to be taken care of */
  result = Cudd_T((DdNode *)f);
  common_error(result, "bdd_then: result = NULL");
//  Cudd_Ref(result);
  return((bdd_ptr)result);
}

bdd_ptr	bdd_else (bdd_ptr f) {
  DdNode *result;

  /* ref is to be taken care of */
  result = Cudd_E((DdNode *)f);
  common_error(result, "bdd_then: result = NULL");
//  Cudd_Ref(result);
  return((bdd_ptr)result);
}

int	bdd_value (bdd_ptr f) {
  int result;

  /* ref is to be taken care of */
  result = (int)Cudd_V((DdNode *)f);
  return result;
}

int	bdd_is_constant (bdd_ptr f) {
  int result;

  /* ref is to be taken care of */
  result = (int)Cudd_IsConstant((DdNode *)f);
  return result;
}
...
bdd_ptr bdd_compose(DdManager * dd, bdd_ptr f, bdd_ptr var, int ind)
{
  DdNode * result;

  result = Cudd_bddCompose(dd,f,var,ind); 
  common_error(result, "bdd_compose: result = NULL");
  Cudd_Ref(result);
  return((bdd_ptr)result);
}

bdd_ptr bdd_and_abstract(DdManager * dd, bdd_ptr f, bdd_ptr g, bdd_ptr cube)
{
  DdNode * result;

  result = Cudd_bddAndAbstract(dd,f,g,cube);
